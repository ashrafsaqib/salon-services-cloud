...
export function SlotSelectionStep({ groups, services, selectedSlots, onSlotSelect }: SlotSelectionStepProps) {
  return (
    <div className="space-y-10">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Select Time Slot</h2>
        <p className="text-gray-600">Choose your preferred time and professional for each service group</p>
      </div>
      {groups.map((group, idx) => {
        // Find service details for this group
        const groupServices = (group.services || []).map((sid: number) => services.find((s: any) => s.id === sid)).filter(Boolean)
        const selected = selectedSlots[idx] || {}
        return (
          <Card key={idx} className="border-2 border-rose-100">
            <CardContent className="p-6">
              <div className="mb-4">
                <div className="font-semibold text-lg text-rose-700 mb-1">Service{groupServices.length > 1 ? 's' : ''}:</div>
                ...
                {(group.slots || []).map((slot: any) => (
                  (slot.staff || []).map((staff: any) => (
                    <Button
                      ...
                      variant={selected.slot?.id === slot.id && selected.staff?.id === staff.id ? "default" : "outline"}
                      ...
                      className={`p-0 h-auto rounded-xl shadow-sm border transition-all flex-col items-stretch text-left overflow-hidden ${
                        selected.slot?.id === slot.id && selected.staff?.id === staff.id
